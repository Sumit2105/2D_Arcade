...
namespace GameProject
{
    ...
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        protected override void Update(GameTime gameTime)
        {
                ...
                for (int j = i + 1; j < bears.Count; j++)
                {
                        ...
                        bears[i].CollisionRectangle.Intersects(bears[j].CollisionRectangle))
                    {
                        ...
                        var = CollisionUtils.CheckCollision(elapsedSpawnDelayMilliseconds, 
                            GameConstants.WINDOW_WIDTH, GameConstants.WINDOW_HEIGHT, bears[i].Velocity,
                        ...
                        if (var != null)
                        {
                            ...
                            {
                                bears[i].Velocity = var.FirstVelocity;
                                bears[i].DrawRectangle = var.FirstDrawRectangle;
                            }
                        }
                        if (var != null)
                        {
                            ...
                            {
                                bears[j].Velocity = var.SecondVelocity;
                                bears[j].DrawRectangle = var.SecondDrawRectangle;
                            }
                        }
            ...
            foreach (TeddyBear bear in bears)
            {
                if (bear.Active==true && bear.CollisionRectangle.Intersects(burger.CollisionRectangle))
                {
                    burgerDamage.Play();
                    burger.Health_property = burger.Health_property - GameConstants.BEAR_DAMAGE;
                    bear.Active = false;
            ...
            foreach (Projectile projectile in projectiles)
            {
                    if ((projectile.Type == ProjectileType.TeddyBear) && (projectile.Active == true) &&
                        (projectile.CollisionRectangle.Intersects(burger.CollisionRectangle)))
                    {
                        burgerDamage.Play();
                        burger.Health_property = burger.Health_property - GameConstants.TEDDY_BEAR_PROJECTILE_DAMAGE;
                ...
                foreach (TeddyBear bear in bears)
                {
                    foreach (Projectile projectile in projectiles)
                    {
                        if (projectile.Type==ProjectileType.FrenchFries && bear.Active==true && projectile.Active==true &&
        ...
        {
            if (burger.Health_property == 0 && burgerDead == false)
            {
                burgerDead = true;
                burgerDeath.Play();
